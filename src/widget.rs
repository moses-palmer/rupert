...
use crate::parse::{Section, Sections};
        ...
        let parts = Layout::default()
            ...
            .constraints(
                self.iter()
                    .enumerate()
                    .map(|(i, section)| {
                        // Add 1 the the size fo all but the last item
                        Constraint::Length(
                            section.height(area.width)
                                + if i < self.len() - 1 { 1 } else { 0 },
                        )
                    })
                    .collect::<Vec<_>>(),
            )
            .split(area);
        for (part, section) in parts.into_iter().zip(self.iter()) {
...
impl<'a> Section<'a> {
    ...
    fn height_block_quote(width: u16, content: &Sections<'a>) -> u16 {
        ...
        2 + content
            .iter()
            .map(|section| section.height(width))
            .sum::<u16>()
    }
    ...
    fn height_list_item_ordered(
        ...
        content.iter().map(|section| section.height(width)).sum()
    }
    ...
    fn height_paragraph(width: u16, text: &Text<'a>) -> u16 {
        // The height of a paragraph is the height of its wrapped lines
        text.lines
            .iter()
