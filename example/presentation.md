...
[commands.initialize]
binary = "./initialize"
arguments = ["${presentation.path}"]

[commands.update]
binary = "./update"
arguments = ["${presentation.path}", "${page.current}"]
...
```makefile
../target/presentation.pdf: presentation.md
    @awk '\
        BEGIN { meta = 0 } \
        { if (meta > 1) print } \
        /^%%%$$/ { meta += 1 }' \
...
run that command. On _Ubuntu_, the following packages are required:

* `libpoppler-glib-dev`
* `python3-gi-cairo`

The PDF viewer provides a _D-Bus_ interface that we can use to synchronise the
page displayed by _rupert_ with the external viewer.
